...
func main() {
	...
	// usage error is there is not one or two remaining arguments
	if flag.NArg() < 1 && flag.NArg() > 2 {
		fmt.Println("DEBUG", flag.NArg())
		flag.Usage()
		os.Exit(2)
	}
	...
	if flag.NArg() == 2 {
		...
		if err != nil {
			fmt.Printf("Error with ParseQuery %q: %s\n", query, err)
			os.Exit(4)
		}
	}

	resp, err := todoistClient.Get(request, values)
	if err != nil {
		fmt.Printf("Error with Get %q: %s\n", request, err)
		os.Exit(3)
	}
