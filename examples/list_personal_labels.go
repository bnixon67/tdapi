...
func main() {
	...
	personalLabels, err := todoistClient.GetAllPersonalLabels()
	if err != nil {
		fmt.Println("Error retrieving personal labels:", err)
		os.Exit(3)
	}

	fmt.Printf("Personal labels count: %d\n", len(personalLabels))
	for index, personalLabel := range personalLabels {
		labelJSON, err := json.MarshalIndent(personalLabel, "", "  ")
		if err != nil {
			fmt.Println("Error marshaling personal label JSON:", err)
			return
		}
		fmt.Printf("Label %d:\n%s\n", index, labelJSON)
	}
