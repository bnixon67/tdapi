...
func setupFlags(tokenFile *string, scopesString *string) {
	flag.Usage = func() {
		fmt.Fprintf(flag.CommandLine.Output(), "Usage: %s [options] project_id\n", os.Args[0])
...
func main() {
	...
	project_id := flag.Arg(0)

	scopes := strings.Split(scopesString, ",")
	todoistClient := tdapi.New(tokenFile, clientID, clientSecret, scopes)

	project, err := todoistClient.GetProject(project_id)
	if err != nil {
		fmt.Printf("Error retrieving project %q: %s\n", project_id, err)
		os.Exit(3)
	}
